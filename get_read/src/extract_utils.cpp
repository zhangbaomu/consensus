...
{
    static bool is_actg_only(const std::string &seq)
    {
        for (char c : seq)
        {
            switch (c)
    ...
    bool is_standard_human_chrom(const std::string &chrom)
    {
        ...
        return standard.find(chrom) != standard.end();
    }
    ...
    void BatchRegionProcessor::ensure_directory(const std::string &path)
    {
        struct stat st = {0};
    ...
    BatchRegionProcessor::ChromOutputFiles &BatchRegionProcessor::get_or_create_chrom_writers(const std::string &chrom)
    {
        auto it = chrom_writers.find(chrom);
        if (it != chrom_writers.end())
        {
            return it->second;
        }

        ChromOutputFiles writers;
        std::string chrom_dir = base_output_dir + "/" + chrom;
        ensure_directory(chrom_dir);

        std::string fastq_path = chrom_dir + "/" + chrom + "_reads.fastq";
        std::string tsv_path = chrom_dir + "/" + chrom + "_truncation.tsv";

        writers.fastq.reset(new std::ofstream(fastq_path, std::ios::out | std::ios::trunc));
        writers.tsv.reset(new std::ofstream(tsv_path, std::ios::out | std::ios::trunc));

        if (!writers.fastq || !writers.tsv || !writers.fastq->is_open() || !writers.tsv->is_open())
        {
            throw std::runtime_error("Failed to open output files for " + chrom);
        }

        // 写入 TSV header
        (*writers.tsv) << "region_name\tread_id\ttruncated_start\ttruncated_end\tflag\n";

        if (consensus_enabled)
        {
            std::string consensus_path = chrom_dir + "/" + chrom + "_consensus.fasta";
            writers.consensus.reset(new std::ofstream(consensus_path, std::ios::out | std::ios::trunc));
            if (!writers.consensus || !writers.consensus->is_open())
            {
                throw std::runtime_error("Failed to open consensus output for " + chrom);
            }
        }

        chrom_writers.emplace(chrom, std::move(writers));
        return chrom_writers.find(chrom)->second;
    }
    ...
    bool BatchRegionProcessor::load_regions_from_file(const std::string &regions_file)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            try
            {
                ...
                if (!allowed_chroms.empty() && allowed_chroms.find(region.chrom) == allowed_chroms.end())
                {
                    continue; // 按指定染色体过滤
                }
                if (!is_standard_human_chrom(region.chrom))
                {
                    continue; // 跳过非 chr1-22/X/Y 的窗口
                }

                region.use_ref_in_poa = false;
                region.output_dir = base_output_dir + "/" + region.chrom;
                ensure_directory(region.output_dir);
    ...
    bool BatchRegionProcessor::extract_all_reads_from_bam(const char *bam_file)
    {
        ...
        while (sam_read1(in, header, b) >= 0)
        {
            ...
            // 只处理标准人类染色体，其他直接跳过
            if (!is_standard_human_chrom(std::string(chrom_cstr)))
            {
                continue;
            }
    ...
    bool BatchRegionProcessor::process_all_regions()
    {
        ...
        for (size_t i = 0; i < regions.size(); ++i)
        {
            ...
            // 获取参考序列，不存在则跳过整个 region
            std::string reference_hap1, reference_hap2;
            bool has_reference = get_reference_sequences_with_spikein(region, reference_hap1, reference_hap2);
            if (!has_reference)
            {
                regions_without_reference++;
                std::cout << "Skip region " << region.region_str << " because reference is missing." << std::endl;
                continue;
            }
            if (!is_actg_only(reference_hap1) || !is_actg_only(reference_hap2))
            {
                regions_ref_non_actg++;
                std::cout << "Skip region " << region.region_str << " because reference contains non-ACTG bases." << std::endl;
                continue;
            }
            ...
            // 获取对应染色体的输出句柄
            ChromOutputFiles *writers_ptr = nullptr;
            try
            {
                writers_ptr = &get_or_create_chrom_writers(region.chrom);
            }
            catch (const std::exception &e)
            {
                std::cerr << "Failed to prepare output files for " << region.chrom << ": " << e.what() << std::endl;
                continue;
            }
            ChromOutputFiles &writers = *writers_ptr;

            std::vector<std::string> sequences;
            if (consensus_enabled)
            {
                sequences.reserve(reads_map.size() + 1);
            }
            ...
            for (const auto &read_pair : reads_map)
            {
                ...
                // 写入FASTQ
                (*writers.fastq) << "@" << read_id << "\n"
                                 << final_seq << "\n+\n"
                                 << final_qual << "\n";

                // 写入TSV
                (*writers.tsv) << region.safe_name << "\t" << read_id << "\t"
                               << info.raw_read_start << "\t" << info.raw_read_end << "\t"
                               << static_cast<int>(info.flag) << "\n";
            ...
            // 将参考序列作为最后两个read添加（hap1/hap2）
            std::string ref1_id = "REFERENCE_" + region.safe_name + "_hap1";
            std::string ref2_id = "REFERENCE_" + region.safe_name + "_hap2";
            std::string ref1_qual(reference_hap1.length(), 'I');
            std::string ref2_qual(reference_hap2.length(), 'I');

            (*writers.fastq) << "@" << ref1_id << "\n"
                             << reference_hap1 << "\n+\n"
                             << ref1_qual << "\n";
            (*writers.fastq) << "@" << ref2_id << "\n"
                             << reference_hap2 << "\n+\n"
                             << ref2_qual << "\n";

            (*writers.tsv) << region.safe_name << "\t" << ref1_id << "\t"
                           << 0 << "\t" << reference_hap1.length() << "\t0\n";
            (*writers.tsv) << region.safe_name << "\t" << ref2_id << "\t"
                           << 0 << "\t" << reference_hap2.length() << "\t0\n";

            if (consensus_enabled && region.use_ref_in_poa)
            {
                sequences.push_back(reference_hap1);
                sequences.push_back(reference_hap2);
            }
            ...
            if (consensus_enabled)
            {
                if (sequences.size() >= 2)
                {
                    auto poa_result = poa_utils::run_abpoa_on_sequences(sequences);
                    if (poa_result.status == 0 && !poa_result.consensus.empty())
                    {
                        if (writers.consensus)
                        {
                            (*writers.consensus) << ">" << region.safe_name << "\n"
                                                 << poa_result.consensus << "\n";
                        }
                    }
                    else
                    {
    ...
    void BatchRegionProcessor::print_final_statistics()
    {
        ...
        std::cout << "Total regions scheduled: " << regions.size() << std::endl;
        std::cout << "Regions processed: " << processed_regions << std::endl;
        std::cout << "Regions skipped (no reads): " << regions_no_reads << std::endl;
        std::cout << "Regions skipped (no reference): " << regions_without_reference << std::endl;
        std::cout << "Regions skipped (reference non-ACTG): " << regions_ref_non_actg << std::endl;
        if (consensus_enabled)
        {
            std::cout << "Consensus failures: " << consensus_failures << std::endl;
        }
    ...
    bool BatchRegionProcessor::load_variants_from_vcf(const std::string &vcf_gz_path)
    {
        ...
        while (hts_getline(fp, KS_SEP_LINE, &ks) >= 0)
        {
            ...
            // 仅处理分相 GT（a|b），分别构造 hap1/hap2 ALT
            size_t bar = gt.find('|');
            if (bar == std::string::npos)
            {
                skipped++;
            ...
            int a1 = -1, a2 = -1;
            try { a1 = std::stoi(gt.substr(0, bar)); } catch (...) {}
            try { a2 = std::stoi(gt.substr(bar + 1)); } catch (...) {}
            ...
            auto pick_alt = [&](int idx) -> std::string {
                if (idx <= 0) return "";
                if (idx > (int)alts.size()) return "";
                std::string alt = alts[idx - 1];
                if (alt == "*") alt.clear();
                return alt;
            };
            std::string alt_h1 = pick_alt(a1);
            std::string alt_h2 = pick_alt(a2);
            if (alt_h1.empty() && alt_h2.empty())
            {
                skipped++;
                continue;
            }
            ...
            // 记录（按染色体分桶，稍后排序）
            variants_by_chrom[chrom].push_back(Variant{pos1, ref, alt_h1, alt_h2});
            kept++;
        }
        if (ks.s)
            free(ks.s);
        ...
        std::cout << "[SPIKEIN] VCF loaded (phased hap1/hap2): kept=" << kept << ", skipped=" << skipped
                                              ...
                                              int hap_index)
    {
        ...
        for (auto vit = lower; vit != vec.end(); ++vit)
        {
            ...
            const std::string &alt_pick = (hap_index == 0) ? var.alt_h1 : var.alt_h2;
            if (alt_pick.empty())
                continue;
    ...
    bool BatchRegionProcessor::get_reference_sequences_with_spikein(const RegionInfo &region, std::string &hap1, std::string &hap2)
    {
        ...
        hap1 = seq;
        hap2 = seq;
        apply_spikeins(region.chrom, region.start, region.end, hap1, 0);
        apply_spikeins(region.chrom, region.start, region.end, hap2, 1);
        return true;
    }
